 ...
 */
abstract class Noakes_Menu_Manager_Field extends Noakes_Menu_Manager_Wrapper
{
	/**
	 ...
	 * @var    array
	 */
	protected $_array_only = array('attributes', 'conditions', 'fields', 'validation', 'value_collection', 'wrapper_attributes');
	...
	public function __construct($properties = array())
	{
		...
		{
			$this->{$name} = Noakes_Menu_Manager_Utilities::check_array($this->{$name});
		}
	}
		...
		switch ($name)
		{
			...
			case 'input_attributes':
				...
				if (!empty($this->id))
				{
					...
					? ' id="nmm-' . $this->id . '" name="' . $this->id . '"'
					: ' data-nmm-identifier="' . $this->id . '"';
				}
			...
			case 'label_attribute':
				...
				? ' for="nmm-' . $this->id . '"'
				: ' data-nmm-identifier="nmm-' . $this->id . '"';
			...
			case 'sanitization':
				...
				return Noakes_Menu_Manager_Sanitization::TEXT;
	...
	protected function _output($field, $type, $echo)
	{
		...
		if (!empty($field))
		{
			...
			$condition_attr = ' data-nmm-field="';
			$condition_replacement = $condition_attr . '__f__"';
			
			$this->push('wrapper_classes', 'nmm-field');
			$this->push('wrapper_classes', 'nmm-field-' . sanitize_key($type));
			
			if ($this->is_template)
			{
				$this->push('wrapper_classes', 'nmm-field-template');
			}
			...
			if (!$this->hide_labels)
			{
				...
				{
					$description = '<div class="nmm-description">'
				...
				if (!empty($this->label))
				{
					$label = '<div class="nmm-field-label">'
						. '<label' . $this->label_attribute . '><strong>' . Noakes_Menu_Manager_Output::required_asterisk($this->label, $this->validation) . '</strong></label>'
						. $label_description
					. '</div>';
				}
			}
			...
			if (!empty($this->conditions))
			{
				foreach ($this->conditions as $condition)
				{
					...
					(
						...
						&&
						Noakes_Menu_Manager_Utilities::starts_with('Noakes_Menu_Manager_Field_', get_class($condition['field']))
						&&
					...
					)
					{
						...
						$condition_output = '<div class="nmm-hidden nmm-condition" data-nmm-compare="' . esc_attr($condition['compare']) . '" data-nmm-conditional="' . $this->id . '"' . $condition_replacement . ' data-nmm-value="' . esc_attr($condition['value']) . '"></div>';
	...
	/**
	 * Build the validation rules for the field.
	 *
	 * @since 3.0.0
	 *
	 * @access public
	...
	public function build_validation()
	{
		...
		{
			$this->push('validation', true, 'required');
		}
		...
		{
			$this->base->cache->push('form_validation', $this->validation, $this->id);
		}
	}
