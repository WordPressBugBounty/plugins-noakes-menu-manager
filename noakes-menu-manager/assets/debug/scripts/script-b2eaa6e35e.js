...
(function ($)
{
		...
		$.extend(METHODS,
	{
		...
		"ajax_data": function (response)
		{
			if (response.data)
			{
					...
					if (response.data.url)
				{
					...
					window.location = METHODS.sanitize_url(response.data.url);
				}
		...
		{
			var a = $('<a />').attr('href', url).nmm_sanitize_html();
...

				{
...

					{
...

						{
							...
							item.nmm_sanitize_html().insertBefore(insert_before);
...

						{
							...
							item.nmm_sanitize_html().insertBefore(template);
...

					{
...

						{
							...
							prev.nmm_sanitize_html().insertAfter(parent).parent().triggerHandler(EVENTS.sort);
...

					{
...

						{
							...
							next.nmm_sanitize_html().insertBefore(parent).parent().triggerHandler(EVENTS.sort);
